...
// (`upwell::daemon::*`), so the facade root stays free of plugin-specific names.
...
// ---------------------------------------------------------------------------
pub use upwell_macros::{app, component, config, daemon, injectable, methods};
...
#[cfg(not(target_family = "wasm"))]
pub mod config {
    pub use upwell_config::{Dynamic, Format, FormatId, Toml};
...
pub mod daemon {
    ...
    /// plugin types at `::upwell::daemon::*` and core types at `::upwell::*`.
...
pub mod axum {
    ...
    /// code roots plugin types at `::upwell::axum::*` and core types at `::upwell::*`.
    pub use upwell_axum::*;
    #[cfg(feature = "json-ws")]
