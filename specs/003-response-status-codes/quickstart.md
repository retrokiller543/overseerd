...
| Existing handler unchanged | US1 / SC-003 | A handler returning `Result<T, upwell_core::Error>` still compiles and maps to `Internal`. |
...
Run: `cargo test` (optionally `cargo test -p upwell-core status` to scope).
