...
Target: one `#[distributed_slice]` per descriptor kind, declared in `upwell-core` and re-exported
...
- `#[distributed_slice]` elements must be `const`/`'static` (descriptors already are — they are
  `Copy` statics today).
- The macro crate must emit `#[upwell::COMPONENTS]`-style element attributes referencing the
  facade path; `paths::upwell_path` already centralises this.
