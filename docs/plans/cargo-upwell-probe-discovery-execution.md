# Cargo Upwell Probe Discovery And Execution
...
Establish the Cargo-side foundation shared by every `cargo upwell` command and by editor
...
```text
<cargo-target-directory>/upwell/build
```
...
```text
<cargo-target-directory>/upwell/probes
```
...
- Add the `cargo-upwell` workspace crate as a reusable library.
...
- `cargo fmt --all -- --check`
- focused `cargo nextest run -p cargo-upwell`
