# Upwell Project Vision

## Vision Statement

Upwell is a Rust framework for building long-running daemon services from
...
and runtime assembly points while Upwell handles service discovery, dependency
...
Upwell should sit between low-level runtime libraries and fully managed
...
Upwell should reduce that repetition by using metadata-driven generation and
...
Upwell discovers and registers metadata describing those components, services,
...
The important constraint is that Upwell does not take ownership of `main`.
...
Upwell should never require ownership of application startup. Runtime helpers
...
This keeps Upwell usable in real services where startup order, observability
...
Upwell can reduce boilerplate, but it must not make service behavior opaque.

### 4. Metadata First

Upwell's procedural macros should primarily generate metadata and descriptors
...
Public daemon APIs should be described with Rust types. Upwell should use those
...
The first versions of Upwell should focus on a coherent daemon foundation that
...
A future Upwell stack may include:

- `upwell-core`: component and service descriptors, dependency graph metadata,
  daemon builder traits, lifecycle types, shutdown coordination, task supervision
  primitives, and runtime introspection models
- `upwell-runtime`: Tokio-oriented runtime integration, daemon execution,
  lifecycle orchestration, and default implementations
- `upwell-macros`: `#[component]`, `#[service]`, `#[rpc]`, and related metadata
  generation macros
- `upwell-ipc`: Unix socket transport and transport abstraction points
- `upwell-client`: generated or derived Rust client SDK support
- `upwell-observability`: optional tracing, metrics, health, and readiness
  helpers once the core metadata model is proven
- `upwell-config`: optional configuration loading and reload helpers
- `upwell-jobs`: optional background job registration, progress, cancellation,
  and status tracking
