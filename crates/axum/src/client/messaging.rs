//! The protocol-generic pub/sub client capabilities: point-to-point *messages* and topic
//! *subscription*.
//!
...
//! [`WebsocketClientProtocol`](crate::client::WebsocketClientProtocol) (one reply per request). A
//! transport (STOMP's [`StompClientTransport`](crate::client::StompClientTransport), or another
//! protocol's) implements them for its protocol tag. They live behind `feature = "ws"` (not
//! `stomp`), so a non-STOMP protocol's client reuses them.
