...
#[cfg(all(feature = "ws", not(target_family = "wasm")))]
pub use upwell_di::{BoxedComponent, Component, FromContainer, Injectable, Provide, Wiring};

#[cfg(all(feature = "ws", not(target_family = "wasm")))]
pub use upwell_core::TypeDescriptor;
...
/// The axum app type: an [`App`](upwell_app::App) specialized to [`AxumPlugin`].
/// `App::builder(name)` resolves through this alias without a turbofish.
#[cfg(not(target_family = "wasm"))]
pub type App = upwell_app::App<AxumPlugin>;

/// The axum app builder: [`AppBuilder`](upwell_app::AppBuilder) specialized to [`AxumPlugin`].
#[cfg(not(target_family = "wasm"))]
pub type AppBuilder = upwell_app::AppBuilder<AxumPlugin>;
