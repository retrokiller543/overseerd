//! Build-script helper for Upwell's registration-backend seam.
//!
...
//! ones (one custom section per per-type slice). That choice is gated on the `upwell_hybrid`
...
//! target — correct for native builds; cross-compiling to a Mach-O target needs the `UPWELL_HYBRID`
//! force (or the `hybrid-registry` feature the macro reads directly).

use std::env;

/// Registers the `upwell_hybrid` cfg and decides whether to activate it.
///
/// The `UPWELL_HYBRID` environment variable is a **tri-state** override:
...
pub fn configure() {
    ...
    let enable = match env::var("UPWELL_HYBRID").ok().as_deref() {
        Some("0" | "false" | "off" | "no" | "") => false,
        Some(_) => true,
        None => mach_o,
    };

    println!("cargo::rustc-check-cfg=cfg(upwell_hybrid)");
    println!("cargo::rerun-if-env-changed=UPWELL_HYBRID");

    if enable {
        println!("cargo::rustc-cfg=upwell_hybrid");
    }
}
