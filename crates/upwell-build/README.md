...
Build-script helper for the Upwell registration-backend seam.
...
(one shared constructor section) — selected by the `upwell_hybrid` cfg.
...
code (`upwell-macros-core`, `upwell-di`, `upwell-rpc`, `upwell-axum`). It is not meant
for downstream user crates: user code never references `upwell_hybrid`, because the proc-macros
...
to macOS from Linux), set `UPWELL_HYBRID=1`, or enable the `hybrid-registry` feature on the
`upwell` facade (which the macro crate reads directly).
