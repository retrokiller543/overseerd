...
> The Upwell native RPC protocol plugin.
...
| `facade` | Set by the `upwell` facade: root the macros' generated plugin types at `::upwell::daemon::*`. Off (the default) keeps them at `::upwell_rpc::*` so depending on this crate directly works. |
