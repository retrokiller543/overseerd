...
/// Cargo Upwell command represented by a report.
...
/// Stable process exit categories for Cargo Upwell commands.
...
/// One stable health check reported by `cargo upwell doctor`.
#[derive(Clone, Debug, Eq, PartialEq, Serialize)]
...
/// Versioned presentation-neutral result of one Cargo Upwell command.
        ...
        ProbeOutcome::Success { document } => {
            ...
            if command == CommandKind::Doctor {
                ...
                report.checks.push(CommandCheck {
                    code: String::from("cargo-upwell/application-validation"),
                    status,
...
fn add_framework_version_check(report: &mut CommandReport, framework_version: &str) {
    if framework_version == env!("CARGO_PKG_VERSION") {
        report.checks.push(passed_check(
            "cargo-upwell/framework-version",
            "Cargo Upwell and the application framework use the same version.",
        ));
    ...
    report.outcome = CommandOutcome::ValidationFailure;
    report.checks.push(CommandCheck {
        code: String::from("cargo-upwell/framework-version"),
        status: CommandCheckStatus::Failed,
        message: String::from("Cargo Upwell and the application framework versions differ."),
    });
    report.diagnostics.push(Diagnostic {
        code: String::from("cargo-upwell/framework-version-mismatch"),
        severity: DiagnosticSeverity::Error,
        message: format!(
            "Cargo Upwell {} cannot diagnose an application using Upwell {framework_version}.",
...
fn report_error(command: CommandKind, error: ProbeRequestError) -> CommandReport {
    ...
    let (target, diagnostics) = match error {
        ...
        ProbeRequestError::Lock(error) => (
            None,
            vec![tool_diagnostic(
                "cargo-upwell/invocation-lock",
                "Cargo Upwell could not serialize access to its private artifacts.",
                Some(format!(
    ...
    if command == CommandKind::Doctor {
        report.checks.push(CommandCheck {
            code: String::from("cargo-upwell/command"),
            status: CommandCheckStatus::Failed,
