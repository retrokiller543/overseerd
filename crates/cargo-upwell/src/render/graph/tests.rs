use cargo_upwell::{GraphDirection, GraphRelationFamily, GraphSource, GraphView};
use semver::Version;
...
fn declarative_display_labels_text_graph_nodes() {
    ...
    view.nodes[0].display = Some(upwell_tooling_schema::ResourceDisplay {
...
fn mermaid_and_dot_escape_hostile_names_ids_and_labels() {
    ...
    assert!(dot.starts_with("digraph upwell {\n  n0 [label=\""));
    assert!(dot.contains("\\\""));
    assert!(dot.contains("\\\\"));
