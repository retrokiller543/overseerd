...
use upwell_tooling_schema::{CliArgument, CliCommand, Resource, ResourceKind, ToolingDocument};
...
fn write_display(
    display: &upwell_tooling_schema::ResourceDisplay,
    output: &mut dyn io::Write,
) -> io::Result<()> {
...
fn owner_uses_provider(
    owner: &upwell_tooling_schema::CliOwner,
    provider_ids: &BTreeSet<&str>,
) -> bool {
    match owner {
        upwell_tooling_schema::CliOwner::Plugin { provider } => {
