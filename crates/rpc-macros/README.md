...
protocol plugin types (`::upwell::daemon::*` or `::upwell_rpc::*`), they can't live in the
...
`app!`/`daemon!` are **not** here — they are protocol-agnostic core macros in `upwell-macros`,
...
the generated plugin-type root between `::upwell::daemon` and the standalone `::upwell_rpc`.
...
| `facade` | Root generated plugin types at the `upwell` facade (`::upwell::daemon::*`) instead of this protocol crate (`::upwell_rpc::*`). Enabled by the facade; off means standalone `upwell-rpc` is the root. Core vocabulary is always `::upwell` either way. |
