...
fn native_trait_gains_runtime_component_descriptor() {
    ...
    let expanded = compact(expand(item, &Paths::upwell()));
    ...
    assert!(expanded.contains(
        "traitRepository:Send+Sync+::upwell::RuntimeDescriptor<::upwell::ComponentDescriptor>{}"
    ));
}

#[test]
fn runtime_descriptor_uses_overridden_facade_path() {
    let args: InjectableArgs = syn::parse_quote!(upwell = ::framework, crate = ::plugin);
    let item = syn::parse_quote! {
        trait Repository {}
    };
    let expanded = compact(expand(item, &args.paths(Paths::upwell())));
