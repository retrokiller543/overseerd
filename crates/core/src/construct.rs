...
//! argument tuple, the way [`Handler`](crate::extract::Handler) is): it knows its
//! parameters, so it reports its [`DependencyDescriptor`]s and resolves each one
//! from the [`ComponentConstructionContext`] before calling the user's function.
//! [`FromContainer`] is the per-argument extractor it builds on; [`FactoryOutput`]
//! normalizes the return value (a component, or a `Result<component, E>`).
//!
...
//!
//! Factories are async (a non-async constructor is wrapped in an async fn by the
//! macro): coherence forbids one trait spanning both async and sync returns, the
//! same reason `Handler`/`FallibleHandler` are split.
...
fn short_name<T: ?Sized + 'static>() -> &'static str {
...
fn dependency_of<T: ?Sized + 'static>(
...
/// `PeerInfo`, a `#[component(by_value)]` type), [`Cfg<T>`] (the sole config binding
/// of its type), `Option<Arc<T>>` (an optional component), `Vec<Arc<dyn Trait>>`
/// (every provider), and `HashMap<String, Arc<dyn Trait>>` (providers keyed by
/// qualifier). Per-parameter qualifiers/paths are not expressed here (a bare
/// function parameter carries no attribute); for those, use field injection.
...
impl<T> FromContainer for Cfg<T>
where
    T: Send + Sync + 'static,
{
    fn dependency() -> DependencyDescriptor {
...
/// so the compiler can prove they do not overlap — unlike the `Serialize`-based
/// [`Responder`](crate::extract::Responder).
...
/// selects the arity impl (the trick [`Handler`](crate::extract::Handler) uses).
///
