...
use super::source::ConfigSnapshot;
...
impl<T: ConfigProperties> ConfigSlot<T> {
    ...
    pub(crate) fn from_seed(
        ...
        Some(Box::new(ConfigSlot {
            cfg,
            path: path.to_string(),
            snapshot: Arc::new(Mutex::new(manager.snapshot(path))),
        }))
    }
}
...
impl<T: ConfigProperties> ReloadableConfig for ConfigSlot<T> {
    ...
    fn prepare(
        ...
        let next_snapshot = manager.snapshot_in(new_root, &self.path);
        let unchanged = *self
            .snapshot
            .lock()
        ...
        Ok(Some(PreparedSwap {
            ...
            commit: Box::new(move || {
                ...
                *snapshot
                    .lock()
                    .unwrap_or_else(std::sync::PoisonError::into_inner) = next_snapshot;
            }),
        }))
    }
